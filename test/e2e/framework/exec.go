...
func (f *Framework) NamespaceContent() (string, error) {
	...
	cmd := exec.Command("/bin/bash", "-c", fmt.Sprintf("%v get pods,services,endpoints,deployments,ingress,ingressclass --namespace %s -o yaml", KubectlPath, f.Namespace))
	cmd.Stdout = &execOut
	cmd.Stderr = &execErr
...
func (f *Framework) newIngressController(namespace string, namespaceOverlay string) error {
	...
	cmd := exec.Command("./wait-for-nginx.sh", namespace, strconv.FormatBool(Dataplane), namespaceOverlay)
