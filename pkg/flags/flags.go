...
Takes the form "<host>:port". If not provided, no admission controller is started.`)
		...
		insecureEnableConfigValidation = flags.Bool("insecure-enable-config-validation", false,
			`If true, will enable validation of the NGINX configuration. This makes NGINX controller vulnerable to CVE-2025-1974. Defaults to false`)
	)
...
https://blog.maxmind.com/2019/12/significant-changes-to-accessing-and-using-geolite2-databases/ .`)
	...
	config := &controller.Configuration{
		...
		InsecureEnableConfigValidation: *insecureEnableConfigValidation,
