...
func (c *Template) buildServerLocations(server *ingress.Server, locations []*ingress.Location) ngx_crossplane.Directives {
	...
	for _, location := range locations {
		...
		if applyGlobalAuth {
			externalAuth = buildExternalAuth(cfg.GlobalExternalAuth)
		} else {
			externalAuth = buildExternalAuth(location.ExternalAuth)
		}
		if authPath != "" {
			serverLocations = append(serverLocations, c.buildAuthLocation(server, location, authPath, externalAuth, applyAuthUpstream, applyGlobalAuth))
		}
		if location.Denied == nil && externalAuth != nil && externalAuth.SigninURL != "" {
			...
			directives = append(directives,
				buildDirective("return",
					"302",
					buildAuthSignURL(externalAuth.SigninURL, externalAuth.SigninURLRedirectParam)))
