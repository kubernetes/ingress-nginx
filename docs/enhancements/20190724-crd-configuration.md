---
title: Migrate configuration configmap
authors:
  - "@aledbf"
reviewers:
  - "@ElvinEfendi"
...
# Migrate configuration configmap
...
ingress-nginx uses a ConfigMap to store global configuration and default values. This approach worked for a long time, but now we have problems with the restriction of the structure (everything is a string).
The proposal is the migration from a ConfigMap to a CRD that fixes these issues.

## Motivation

The ConfigMap structure does not allow the definition of data types or lists. Also, the validation of the content is more difficult than should be.

### Goals

- Stop using configmaps to configure the ingress controller
- Remove ambiguity in names with prefixes `enable-` or `disable-xxxxx` like `disable-ipv6`.
- Use types:
  - for nginx sizes
  - booleans for settings `on|off`
  - lists like whitelists, protocols, ciphers, etc.

### Non-Goals

## Proposal

- Define a CRD: `configuration.ingress-nginx.io`
- Simplify the validation of values
- Provide defaults (when necessary) for values not defined in the created configuration
- Provide a new binary to allow the migration from the configmap to the CRD
  - reads configuration, tcp, and udp configmap
  - returns a configuration CRD using the same namespace and name of configuration configmap
- Replace
- Deprecate flags `--configmap`, `--tcp-services-configmap` and `--udp-services-configmap`
- Reuse the value defined in the flag `--configmap` to configure the new one `--configuration` to don't require an update of existing deployments
...
This proposal is the alternative :)
