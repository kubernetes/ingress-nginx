...
| `--ingress-class-by-name`          | Allows NGINX ingress controller to additionally watch for ingressClass which matches the name set by `--ingress-class` flag but points to a different controller. (default false) |
