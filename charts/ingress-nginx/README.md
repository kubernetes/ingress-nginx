...
| controller.disablePathTypeValidation | bool | `false` | This configuration defines if Ingress Controller should validate pathType and allow special characters only when pathType is ImplementationSpecific. If true, the validation will be disabled and special characters will be allowed on any pathType. |
